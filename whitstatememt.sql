select E.EmployeeNumber from employee as E 
...
--Compara los números generados (U.RowNumber) con los EmployeeNumber reales en la tabla.

--Si un número no existe en tblTransaction, aparecerá como NULL en el join.

--Por tanto, devuelve los EmployeeNumber faltantes.
select row_number() over(order by(select null)) from sys.objects O cross join sys.objects P
--Te genera una lista de números consecutivos desde 1 hasta varios miles sin necesidad de una tabla auxiliar
go
with Numbers as (
select top(select max(EmployeeNumber) from tblTransaction) row_Number() over(order by (select null)) as RowNumber
from tblTransaction as U),
